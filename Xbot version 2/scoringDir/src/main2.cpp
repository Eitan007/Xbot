...
#include <codecvt>
#include <locale>
#include <C:/Users/USER/Desktop/vcpkg/installed/x64-windows-static/include/xlnt/xlnt.hpp>
// #include <C:/Users/USER/Desktop/vcpkg/installed/x64-windows-static/include/xlnt/xlnt_config.hpp>
...
void scoreData_0(const std::string& input_file_path, const std::string& output_file_path, double scoring_factor) {
...
void scoreData(const std::string& input_file_path, const std::string& output_file_path, double scoring_factor) {
    ...
    for (auto row : worksheet.rows(false)) {
        ...
        if (row[0].has_value()) data_row.name = row[0].to_string();
        if (row[1].has_value()) data_row.username = row[1].to_string();
        if (row[2].has_value()) data_row.url = row[2].to_string();
        if (row[3].has_value()) data_row.followers = row[3].to_number();
        if (row[4].has_value()) data_row.following = row[4].to_int();
        if (row[5].has_value()) data_row.tweets = row[5].to_int();
        if (row[6].has_value()) data_row.bio = row[6].to_string();
        if (row[7].has_value()) data_row.canDM = (row[7].to_string() == "1");
        if (row[8].has_value()) data_row.location = row[8].to_string();
        if (row[9].has_value()) data_row.joinedX = row[9].to_string();
        if (row[10].has_value()) data_row.translator = (row[10].to_string() == "1");
        if (row[11].has_value()) data_row.likes = row[11].to_int();
        if (row[12].has_value()) data_row.bluetick = (row[12].to_string() == "1");
        if (row[13].has_value()) data_row.defaultpic = (row[13].to_string() == "1");
        if (row[14].has_value()) data_row.rawScore = row[14].to_int();
        if (row[15].has_value()) data_row.totalScore = row[15].to_int();
        ...
        // Apply deductions and calculate scores
        data_row.rawScore = apply_deduction(data_row);
        data_row.totalScore = calculate_final_score(data_row, scoring_factor);
